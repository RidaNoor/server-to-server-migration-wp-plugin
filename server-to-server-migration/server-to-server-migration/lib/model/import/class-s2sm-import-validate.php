...
_Import_Validate {
	...
	public static function execute( $params ) {
		...
		// Set progress
		S2sm
_Status::info( __( 'Unpacking archive...', S2SM
_PLUGIN_NAME ) );
...
_Not_Accesible_Exception(
				sprintf( __( 'Unable to get the file size of <strong>%s</strong>', S2SM
_PLUGIN_NAME ), $name )
			);
		}
...
_storage_path( $params ), array( S2SM
_PACKAGE_NAME, S2SM
_MULTISITE_NAME, S2SM
_DATABASE_NAME ), $content_offset, 10 ) ) ) {
...
_package_path( $params ) ) ) {
				throw new S2sm
_Import_Exception(
					__( 'Invalid archive file. It should contain <strong>package.json</strong> file.', S2SM
_PLUGIN_NAME )
				);
			}
