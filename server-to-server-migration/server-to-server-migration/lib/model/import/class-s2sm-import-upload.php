...
_PLUGIN_NAME ),
		...
		if ( ! array_key_exists( 'error', $_FILES['upload-file'] ) ) {
			throw new S2sm
_Import_Retry_Exception(
				__( 'Missing error key in upload file.', S2SM
_PLUGIN_NAME ),
				400
			);
		}
		...
		if ( ! array_key_exists( 'tmp_name', $_FILES['upload-file'] ) ) {
			throw new S2sm
_Import_Retry_Exception(
				__( 'Missing tmp_name in upload file.', S2SM
_PLUGIN_NAME ),
				400
			);
		}
...
_Import_Retry_Exception(
				sprintf(
					__( 'Unable to fetch file because %s', S2SM
_PLUGIN_NAME ),
					$e->getMessage()
				),
				400
...
_Import_Retry_Exception(
						sprintf(
							__( 'Unable to upload the file because %s', S2SM
_PLUGIN_NAME ),
							$e->getMessage()
						),
						400
			...
			case UPLOAD_ERR_NO_FILE:
				...
				throw new S2sm
_Import_Retry_Exception(
					__( 'The file is too large, retrying with smaller size.', S2SM
_PLUGIN_NAME ),
					413
				);
			case UPLOAD_ERR_NO_TMP_DIR:
				throw new S2sm
_Import_Retry_Exception(
					__( 'Missing a temporary folder.', S2SM
_PLUGIN_NAME ),
					400
				);
			case UPLOAD_ERR_CANT_WRITE:
				throw new S2sm
_Import_Retry_Exception(
					__( 'Failed to write file to disk.', S2SM
_PLUGIN_NAME ),
					400
				);
			case UPLOAD_ERR_EXTENSION:
				throw new S2sm
_Import_Retry_Exception(
					__( 'A PHP extension stopped the file upload.', S2SM
_PLUGIN_NAME ),
					400
				);
			default:
...
_Import_Retry_Exception(
					sprintf(
						__( 'Unrecognized error %s during upload.', S2SM
_PLUGIN_NAME ),
						$error
					),
					400
