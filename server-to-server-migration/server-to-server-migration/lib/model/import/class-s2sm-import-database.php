...
_close( $handle );
		...
		// Set progress
		S2sm
_Status::info( __( 'Restoring database...', S2SM
_PLUGIN_NAME ) );
...
_uploads_path( $blog['New']['BlogID'] ), '\/' ) );
		...
		// Get URL IP
		$url_ip = get_option( S2SM
_URL_IP );

		// Get URL adapter
		$url_adapter = get_option( S2SM
_URL_ADAPTER );
...
_SECRET_KEY );
		...
		// Get HTTP user
		$auth_user = get_option( S2SM
_AUTH_USER );

		// Get HTTP password
		$auth_password = get_option( S2SM
_AUTH_PASSWORD );
...
_cache_flush();
		...
		// Set progress
		S2sm
_Status::info( __( 'Done restoring database...', S2SM
_PLUGIN_NAME ) );
