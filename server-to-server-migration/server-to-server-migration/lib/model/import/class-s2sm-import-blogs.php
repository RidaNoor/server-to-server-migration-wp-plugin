...
_Import_Blogs {
	...
	public static function execute( $params ) {
		...
		// Set progress
		S2sm
_Status::info( __( 'Preparing blogs...', S2SM
_PLUGIN_NAME ) );
			...
			if ( empty( $multisite['Network'] ) ) {
				if ( isset( $multisite['Sites'] ) && ( $sites = $multisite['Sites'] ) ) {
					if ( count( $sites ) === 1 && ( $subsite = current( $sites ) ) ) {
						...
						throw new S2sm
_Import_Exception(
							__( 'The archive should contain <strong>Single WordPress</strong> site! Please revisit your export settings.', S2SM
_PLUGIN_NAME )
						);
					}
				} else {
					throw new S2sm
_Import_Exception(
						__( 'At least <strong>one WordPress</strong> site should be presented in the archive.', S2SM
_PLUGIN_NAME )
					);
				}
			} else {
				throw new S2sm
_Import_Exception(
					__( 'Unable to import <strong>WordPress Network</strong> into WordPress <strong>Single</strong> site.', S2SM
_PLUGIN_NAME )
				);
			}
		}
...
_close( $handle );
		...
		// Set progress
		S2sm
_Status::info( __( 'Done preparing blogs...', S2SM
_PLUGIN_NAME ) );
