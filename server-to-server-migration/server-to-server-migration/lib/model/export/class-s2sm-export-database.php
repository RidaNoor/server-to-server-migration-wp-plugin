...
_Export_Database {
	...
	public static function execute( $params ) {
		...
		// Set progress
		S2sm
_Status::info( __( 'Exporting database...', S2SM
_PLUGIN_NAME ) );
...
_table_prefix() . 'usermeta', array( 'meta_key' ) );
		...
		$client->set_table_query_clauses( s2sm
_table_prefix() . 'options', sprintf( " WHERE option_name NOT IN ('%s', '%s') ", S2SM
...
_database_path( $params ), S2SM
_DATABASE_NAME );
			$archive->close();

			// Set progress
			S2sm
_Status::info( __( 'Done exporting database.', S2SM
_PLUGIN_NAME ) );
