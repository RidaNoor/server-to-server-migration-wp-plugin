...
-report-problem-button').click(function (e) {
			$(this).next('.s2sm
-report-problem-dialog').toggleClass('s2sm
-report-active');
...
-report-cancel').click(function (e) {
			$(this).closest('.s2sm
-report-problem-dialog').removeClass('s2sm
-report-active');
...
_terms': +terms },
				success: function success(data) {
					...
					if (errors.length > 0) {
						...
						$('.s2sm
-report-problem-dialog .s2sm
-message').remove();

						var content = $('<div />').addClass('s2sm
-message s2sm
-error-message');
						...
						$('.s2sm
-report-problem-dialog').prepend(content);
					} else {
						var _content = $('<div />').addClass('s2sm
-message s2sm
-success-message').append('<p>Thanks for submitting your request!</p>');
						$('.s2sm
-report-problem-dialog').html(_content);
...
/***/ function(module, exports) {
	...
	jQuery(document).ready(function ($) {
		...
		$('.s2sm
-feedback-type').attr('checked', false);

		// Review
		$('#s2sm
-feedback-type-link-1').click(function (e) {
			var radio = $('#s2sm
-feedback-type-1');
			if (radio.is(':checked')) {
		...
		// Improvement
		$('#s2sm
-feedback-type-2').click(function () {
			// Hide other options
			$('#s2sm
-feedback-type-1, #s2sm
-feedback-type-3').closest('li').hide();

			// change placeholder message
			$('.s2sm
-feedback-form').find('.s2sm
-feedback-message').attr('placeholder', 'Leave plugin developers any feedback here');
...
-feedback-form').fadeIn();
		...
		// Help
		$('#s2sm
-feedback-type-3').click(function () {
			// Hide other options
			$('#s2sm
-feedback-type-1, #s2sm
-feedback-type-2').closest('li').hide();

			// change placeholder message
			$('.s2sm
-feedback-form').find('.s2sm
-feedback-message').attr('placeholder', 'How may we help you?');
...
-feedback-form').fadeIn();
		...
		$('#s2sm
-feedback-cancel').click(function (e) {
			$('.s2sm
-feedback-form').fadeOut(function () {
				$('.s2sm
-feedback-type').attr('checked', false).closest('li').show();
			});
			e.preventDefault();
		});
...
-feedback-submit').click(function () {
			var type = $('.s2sm
-feedback-type:checked').val();
			var email = $('.s2sm
-feedback-email').val();
			var message = $('.s2sm
-feedback-message').val();
			var terms = $('.s2sm
-feedback-terms').is(':checked');
...
-error-message');
						...
						$('.s2sm
-feedback').prepend(content);
					} else {
						var _content = $('<div />').addClass('s2sm
-message s2sm
-success-message').append('<p>Thanks for submitting your feedback!</p>');
						$('.s2sm
-feedback').html(_content);
					}
				},
				dataType: 'json',
				async: false
...
/***/ function(module, exports, __webpack_require__) {
	...
	Import.prototype.start = function (options, retries) {
		...
		// Set params
		var params = this.params.concat({ name: 'secret_key', value: s2sm
_import.secret_key });
		...
		// Import
		$.post(s2sm
_import.ajax.url, params, null, 'json').done(function () {
			self.getStatus();
	...
	Import.prototype.run = function (params, retries) {
		...
		// Import
		$.post(s2sm
_import.ajax.url, params, null, 'json').done(function (params) {
			if (params) {
				self.run(params);
			}
		}).fail(function () {
	...
	Import.prototype.confirm = function (options, retries) {
		...
		// Set params
		var params = this.params.concat({ name: 'secret_key', value: s2sm
_import.secret_key }).concat({ name: 'priority', value: 150 });
...
_import.ajax.url, params, null, 'json').done(function () {
	...
	Import.prototype.clean = function (options, retries) {
		...
		// Set params
		var params = this.params.concat({ name: 'secret_key', value: s2sm
_import.secret_key }).concat({ name: 'priority', value: 400 });
...
/***/ function(module, exports) {
	...
	var Modal = function Modal() {
		...
		this.error = function (params) {
			...
			// Create title
			var title = $('<span></span>').addClass('s2sm
-title-red').text('Unable to import');

			// Create close button
			var closeButton = $('<button type="button" class="s2sm
-button-red">Close</button>').on('click', function () {
				self.destroy();
			});
		...
		this.progress = function (params) {
			if (this.view === 'progress') {
				...
				// Create progressBar
				var progressBar = $('<span class="s2sm
-progress-bar"></span>');

				// Create progressBarMeter
				this.progress.progressBarMeter = $('<span class="s2sm
-progress-bar-meter"></span>').width(params.percent + '%');

				// Create progressBarPercent
				this.progress.progressBarPercent = $('<span class="s2sm
-progress-bar-percent">0%</span>').text(params.percent + '%');

				// Create stop import
				var stopButton = $('<button type="button" class="s2sm
-button-red"><i class="s2sm
-icon-notification"></i> Stop import</button>').on('click', function () {
		...
		this.confirm = function (params) {
			...
			// Create warning
			var warning = $('<span class="s2sm
-icon-notification"></span>');

			// Create confirm button
			var confirmButton = $('<button type="button" class="s2sm
-button-green">Continue</button>').on('click', function () {
				$(this).attr('disabled', 'disabled');
				self.onConfirm();
			});
		...
		this.blogs = function (params) {
			...
			// Create title
			var title = $('<span></span>').addClass('s2sm
-title-grey').text(params.title);

			// Create continue button
			var continueButton = $('<button type="button" class="s2sm
-button-green">Continue</button>').on('click', function () {
				$(this).attr('disabled', 'disabled');
		...
		this.info = function (params) {
			...
			// Create loader
			var loader = $('<span class="s2sm
-loader"></span>');

			// Create stop import
			var stopButton = $('<button type="button" class="s2sm
-button-red"><i class="s2sm
-icon-notification"></i> Stop import</button>').attr('disabled', 'disabled');
...
-button-red">Close</button>').on('click', function () {
		...
		// Create the overlay
		this.overlay = $('<div class="s2sm
-overlay"></div>');

		// Create the modal container
		this.modal = $('<div class="s2sm
-modal-container"></div>');
...
/***/ function(module, exports, __webpack_require__) {
	...
	window.S2sm
ExportController = function (provider) {
		this._export = {};
		this._provider = provider;
	};

	window.S2sm
ExportController.prototype.start = function () {
		new Dialog('<strong>All in One WP Migration</strong> is not compatible with current version of ' + '<strong>' + Extensions[this._provider] + '</strong>. Please contact ' + '<a href="mailto:support@migzara.com">support@migzara.com</a> for more details.');
	};

	window.S2sm
ImportController = function (provider) {
		this._import = {};
		this._provider = provider;
	};

	window.S2sm
ImportController.prototype.init = function () {};

	window.S2sm
ImportController.prototype.download = function () {
		new Dialog('<strong>All in One WP Migration</strong> is not compatible with current version of ' + '<strong>' + Extensions[this._provider] + '</strong>. Please contact ' + '<a href="mailto:support@migzara.com">support@migzara.com</a> for more details.');
	};
	...
	jQuery(document).ready(function ($) {
		...
		$('#s2sm
be-import-file-cancel').attr('type', 'button');
		$('#s2sm
be-import-file').attr('type', 'button');

		// Dropbox extension
		$('#s2sm
de-import-file-cancel').attr('type', 'button');
		$('#s2sm
de-import-file').attr('type', 'button');

		// FTP extension
		$('#s2sm
fe-import-file-cancel').attr('type', 'button');
		$('#s2sm
fe-import-file').attr('type', 'button');

		// Google Drive extension
		$('#s2sm
ge-import-file-cancel').attr('type', 'button');
		$('#s2sm
ge-import-file').attr('type', 'button');

		// OneDrive extension
		$('#s2sm
oe-import-file-cancel').attr('type', 'button');
		$('#s2sm
oe-import-file').attr('type', 'button');

		// Amazon S3 extension
		$('#s2sm
se-import-file-cancel').attr('type', 'button');
		$('#s2sm
se-import-file').attr('type', 'button');

		// URL extension
		$('#s2sm
le-import-file-cancel').attr('type', 'button');
		$('#s2sm
le-import-file').attr('type', 'button');
	});
...
-modal-container"></div>');
		...
		// Create paragraph to hold mesage
		this.message = $('<p></p>').addClass('s2sm
-dialog-message').html(text);
		...
		// Create close button
		this.closeButton = $('<button type="button" class="s2sm
-button-red">Close</button>').on('click', function () {
			self.modal.hide();
			self.overlay.hide();
		});
...
/***/ function(module, exports, __webpack_require__) {
	...
	FileUploader.prototype.init = function () {
		...
		var selectElement = $('#ridanoor-url-import-submit');
		var dropElement = $('#s2sm
-drag-drop-area');
...
-drag-over');
	...
	FileUploader.prototype.upload = function ( retries) {
		var _this2 = this;

		var retries = retries || 0;

		//var chunkSize = file.size > s2sm
_uploader.chunk_size ? s2sm
_uploader.chunk_size : file.size;
...
_uploader.url, {
			...
			switch (response.status) {
				case 413:
					s2sm
_uploader.chunk_size = chunkSize / 2;
...
_uploader.params;
	...
	FileUploader.prototype.checkSize = function (file) {
		var size = parseInt(s2sm
_uploader.filters.s2sm
_archive_size, 10);

		if (file.size > size && size !== 0) {
			throw new Error(s2sm
_import.oversize);
		}
	};
...
_import.invalid_extension);
	...
	FileUploader.prototype.onBeforeUpload = function () {
		var self = this;
		var import_url = $('#ridanoor-url-import-field').val();
		var file_name = import_url.split("/");
			file_name = file_name[file_name.length-1];
		var storage = Util.random(12);
		var options = Util.form('#s2sm
-import-form')
			.concat({ name: 'storage', value: storage })
			.concat({ name: 'archive', value: file_name })
			.concat({name:'source_url',value: import_url});
