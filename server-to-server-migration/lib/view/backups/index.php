...
-holder">
				<h1><i class="s2sm
-icon-export"></i> <?php _e( 'Backups', S2SM_PLUGIN_NAME ); ?></h1>
...
-clear">
					...
					<?php if ( is_readable( S2SM_BACKUPS_PATH ) && is_writable( S2SM_BACKUPS_PATH ) ) :  ?>
						<?php if ( $backups ) : ?>
							<table class="s2sm
-backups">
								<thead>
									<tr>
										<th class="s2sm
-column-name"><?php _e( 'Name', S2SM_PLUGIN_NAME ); ?></th>
										<th class="s2sm
-column-date"><?php _e( 'Date', S2SM_PLUGIN_NAME ); ?></th>
										<th class="s2sm
-column-size"><?php _e( 'Size', S2SM_PLUGIN_NAME ); ?></th>
										<th class="s2sm
-column-actions"></th>
									</tr>
								</thead>
...
-column-info" colspan="3">
												<?php _e( 'The file is too large for your hosting plan.', S2SM_PLUGIN_NAME ); ?>
											</td>
										<?php else : ?>
											<td class="s2sm
-column-date">
												<?php echo human_time_diff( $backup['mtime'] ); ?> <?php _e( 'ago', S2SM_PLUGIN_NAME ); ?>
											</td>
											<td class="s2sm
-column-size">
...
-icon-plus2 s2sm
-icon-alone"></i>
													<span><?php _e( 'Get URL', S2SM_PLUGIN_NAME ); ?></span>
												</a>
...
-icon-alone"></i>
													<span><?php _e( 'Download', S2SM_PLUGIN_NAME ); ?></span>
												</a>
...
-icon-alone"></i>
													<span><?php _e( 'Restore', S2SM_PLUGIN_NAME ); ?></span>
												</a>
												<a href="#" data-archive="<?php echo esc_attr( $backup['filename'] ); ?>" class="s2sm
-button-red s2sm
-button-alone s2sm
...
-icon-close s2sm
-icon-alone"></i>
													<span><?php _e( 'Delete', S2SM_PLUGIN_NAME ); ?></span>
												</a>
											</td>
										<?php endif; ?>
									</tr>
						...
						<?php else : ?>
							<div class="s2sm
-backups-empty">
								<p><?php _e( 'There are no backups available at this time, why not create a new one?', S2SM_PLUGIN_NAME ); ?></p>
								<p>
									<a href="<?php echo network_admin_url( 'admin.php?page=site-migration-export' ); ?>" class="s2sm
-button-green">
										<i class="s2sm
-icon-export"></i>
										<?php _e( 'Create backup', S2SM_PLUGIN_NAME ); ?>
									</a>
								</p>
							</div>
						<?php endif; ?>
