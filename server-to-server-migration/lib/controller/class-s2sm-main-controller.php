...
_Main_Controller
	...
	private function activate_actions() {
		...
		add_action( 'plugins_loaded', array( $this, 's2sm
_loaded' ), 20 );
...
_buttons' ), 10 );
	 ...
	 * @return void
	 */
	public function s2sm
_commands() {
		// Add export commands
		add_filter( 's2sm
_export', 'S2sm
_Export_Init::execute', 5 );
		add_filter( 's2sm
_export', 'S2sm
_Export_Compatibility::execute', 5 );
...
_manual_export'] ) ) {
			add_filter( 's2sm
_export', 'S2sm
_Export_Resolve::execute', 5 );
		}

		add_filter( 's2sm
_export', 'S2sm
_Export_Archive::execute', 10 );
		add_filter( 's2sm
_export', 'S2sm
_Export_Config::execute', 50 );
		add_filter( 's2sm
_export', 'S2sm
_Export_Enumerate::execute', 100 );
		add_filter( 's2sm
_export', 'S2sm
_Export_Content::execute', 150 );
		add_filter( 's2sm
_export', 'S2sm
_Export_Database::execute', 200 );
		add_filter( 's2sm
_export', 'S2sm
_Export_Download::execute', 250 );
		add_filter( 's2sm
_export', 'S2sm
_Export_Clean::execute', 300 );

		// Add import commands
		add_filter( 's2sm
_import', 'S2sm
_Import_Upload::execute', 5 );
		add_filter( 's2sm
_import', 'S2sm
_Import_Upload::executeUrl', 7 );
		add_filter( 's2sm
_import', 'S2sm
_Import_Compatibility::execute', 10 );
...
_manual_backups'] ) ) {
			add_filter( 's2sm
_import', 'S2sm
_Import_Resolve::execute', 10 );
		}

		add_filter( 's2sm
_import', 'S2sm
_Import_Validate::execute', 50 );
		add_filter( 's2sm
_import', 'S2sm
_Import_Confirm::execute', 100 );
		add_filter( 's2sm
_import', 'S2sm
_Import_Blogs::execute', 150 );
		add_filter( 's2sm
_import', 'S2sm
_Import_Enumerate::execute', 200 );
		add_filter( 's2sm
_import', 'S2sm
_Import_Content::execute', 250 );
		add_filter( 's2sm
_import', 'S2sm
_Import_Plugins::execute', 270 );
		add_filter( 's2sm
_import', 'S2sm
_Import_Database::execute', 300 );
		add_filter( 's2sm
_import', 'S2sm
_Import_Done::execute', 350 );
		add_filter( 's2sm
_import', 'S2sm
_Import_Clean::execute', 400 );
	}
...
_buttons() {
		// Add export buttons
		add_filter( 's2sm
_export_buttons', 'S2sm
_Export_Controller::buttons' );

		// Add import buttons
		add_filter( 's2sm
_import_buttons', 'S2sm
_Import_Controller::buttons' );
	}
...
_loaded() {
		if ( is_multisite() ) {
			if ( apply_filters( 's2sm
_multisite_menu', false ) ) {
		...
		add_action( 'wp_maybe_auto_update', 'S2sm
_Updater_Controller::check_for_updates' );

		// Add chunk size limit
		add_filter( 's2sm
_max_chunk_size', 'S2sm
_Import_Controller::max_chunk_size' );

		// Add plugins api
		add_filter( 'plugins_api', 'S2sm
_Updater_Controller::plugins_api', 20, 3 );

		// Add plugins updates
		add_filter( 'pre_set_site_transient_update_plugins', 'S2sm
_Updater_Controller::pre_update_plugins' );

		// Add plugins metadata
		add_filter( 'site_transient_update_plugins', 'S2sm
_Updater_Controller::update_plugins' );

		// Add "Check for updates" link to plugin list page
		add_filter( 'plugin_row_meta', 'S2sm
_Updater_Controller::plugin_row_meta', 10, 2 );
	}
	...
	public function multisite_notice() {
		S2sm
_Template::render( 'main/multisite-notice' );
	}
	...
	public function storage_notice() {
		S2sm
_Template::render( 'main/storage-notice' );
	}
	...
	public function index_notice() {
		S2sm
_Template::render( 'main/index-notice' );
	}
	...
	public function backups_notice() {
		S2sm
_Template::render( 'main/backups-notice' );
	}
	...
	public function plugin_row_meta( $links, $file ) {
		if ( $file == S2SM_PLUGIN_BASENAME ) {
			$links[] = S2sm
_Template::get_content( 'main/get-support' );
		}
...
_File_Index::create( S2SM_STORAGE_INDEX );
		...
		S2sm
_File_Index::create( S2SM_BACKUPS_INDEX );

		// Create .htaccess in backups folder
		S2sm
_File_Htaccess::create( S2SM_BACKUPS_HTACCESS );

		// Create web.config in backups folder
		S2sm
_File_Webconfig::create( S2SM_BACKUPS_WEBCONFIG );
	}
...
-css-export',
			S2sm
_Template::asset_link( 'css/export.min.css' )
		);
		wp_enqueue_script(
			's2sm
-js-export',
			S2sm
_Template::asset_link( 'javascript/export.min.js' ),
			array( 'jquery' )
		);
		wp_localize_script( 's2sm
-js-export', 's2sm
_feedback', array(
			'ajax' => array(
				'url' => wp_make_link_relative( admin_url( 'admin-ajax.php?action=s2sm
_feedback' ) ),
			),
		) );
		wp_localize_script( 's2sm
...
-css-import',
			S2sm
_Template::asset_link( 'css/import.min.css' )
		);
		wp_enqueue_script(
			's2sm
-js-import',
			S2sm
_Template::asset_link( 'javascript/import.min.js' ),
			array( 'jquery' )
		);
		wp_localize_script( 's2sm
-js-import', 's2sm
_uploader', array(
			'chunk_size'  => apply_filters( 's2sm
_max_chunk_size', S2SM_MAX_CHUNK_SIZE ),
			'max_retries' => apply_filters( 's2sm
_max_chunk_retries', S2SM_MAX_CHUNK_RETRIES ),
			'url'         => wp_make_link_relative( admin_url( 'admin-ajax.php?action=s2sm
_import' ) ),
			'params'      => array(
				'priority'   => 5,
			...
			'filters'     => array(
				's2sm
_archive_extension' => array( 'wpress', 'bin' ),
				's2sm
_archive_size'      => apply_filters( 's2sm
...
_status' ) ),
			...
			'oversize'   => sprintf(
				...
				size_format( apply_filters( 's2sm
_max_file_size', S2SM_MAX_FILE_SIZE ) )
			),
...
-css-backups',
			S2sm
_Template::asset_link( 'css/backups.min.css' )
		);
		wp_enqueue_script(
			's2sm
-js-backups',
			S2sm
_Template::asset_link( 'javascript/backups.min.js' ),
			array( 'jquery' )
		);
		wp_localize_script( 's2sm
-js-backups', 's2sm
...
-css-updater',
			S2sm
_Template::asset_link( 'css/updater.min.css' )
		);
		wp_enqueue_script(
			's2sm
-js-updater',
			S2sm
_Template::asset_link( 'javascript/updater.min.js' ),
			array( 'jquery' )
		);
		wp_localize_script( 's2sm
-js-updater', 's2sm
_updater', array(
			'ajax' => array(
				'url' => admin_url( 'admin-ajax.php?action=s2sm
_updater' ),
			),
		) );
	}
...
_Template::render( 'main/admin-head', array( 'version' => $wp_version ) );
	...
	public function init() {
		...
		if ( isset( $_GET['s2sm
_updater'] ) ) {
			if ( current_user_can( 'update_plugins' ) && check_admin_referer( 's2sm
_updater_nonce' ) ) {
				S2sm
_Updater::check_for_updates();
	...
	public function router() {
		// Public
		add_action( 'wp_ajax_nopriv_s2sm
_export', 'S2sm
_Export_Controller::export' );
		add_action( 'wp_ajax_nopriv_s2sm
_import', 'S2sm
_Import_Controller::import' );
		add_action( 'wp_ajax_nopriv_s2sm
_status', 'S2sm
_Status_Controller::status' );
		add_action( 'wp_ajax_nopriv_s2sm
_resolve', 'S2sm
_Resolve_Controller::resolve' );

		// Update
		if ( current_user_can( 'update_plugins' ) ) {
			add_action( 'wp_ajax_s2sm
_updater', 'S2sm
_Updater_Controller::updater' );
		}
...
_Import_Controller::import' );
		...
		if ( current_user_can( 'export' ) || current_user_can( 'import' ) ) {
			add_action( 'wp_ajax_s2sm
_backups', 'S2sm
_Backups_Controller::delete' );
			add_action( 'wp_ajax_s2sm
_feedback', 'S2sm
_Feedback_Controller::feedback' );
			add_action( 'wp_ajax_s2sm
_report', 'S2sm
_Report_Controller::report' );
			add_action( 'wp_ajax_s2sm
_status', 'S2sm
_Status_Controller::status' );
			add_action( 'wp_ajax_s2sm
