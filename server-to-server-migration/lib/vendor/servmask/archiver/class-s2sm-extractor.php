...
_Not_Readable_Exception
	...
	public function extract_one_file_to( $location, $exclude = array(), $old_paths = array(), $new_paths = array(), $offset = 0, $timeout = 0 ) {
		if ( false === is_dir( $location ) ) {
			throw new S2sm
_Not_Readable_Exception( sprintf( __( '%s doesn\'t exist', S2SM_PLUGIN_NAME ), $location ) );
		}
	...
	public function set_file_pointer( $handle = null, $offset = 0 ) {
		...
		if ( -1 === $result ) {
			if ( ( $meta = stream_get_meta_data( $handle ) ) ) {
				throw new S2sm
_Not_Accesible_Exception( sprintf( __( 'Unable to seek to offset %d on %s', S2SM_PLUGIN_NAME ), $offset, $meta['uri'] ) );
			}
		}
	}
...
_Not_Accesible_Exception(
				sprintf(
					__( 'Unable to get current pointer position of %s', S2SM_PLUGIN_NAME ),
					$this->filename
				)
			);
		}
