...
_open( $file_name, 'ab' );
		...
		if ( $current_table_index === 0 ) {
			s2sm
_write( $file_handler, $this->get_header() );
		}
		...
		for ( ; $current_table_index < count( $tables ); ) {
			...
			if ( isset( $table['Create Table'] ) ) {
				...
				s2sm
_write( $file_handler, $drop_table );
				...
				s2sm
_write( $file_handler, $create_table );
...
_write( $file_handler, ";\n\n" );
			...
			while ( $row = $this->fetch_assoc( $result ) ) {
				if ( $current_table_offset % S2sm
_Database::QUERIES_PER_TRANSACTION === 0 ) {
					// Write start transaction
					s2sm
_write( $file_handler, "START TRANSACTION;\n" );
				}
				...
				s2sm
_write( $file_handler, $table_insert );
...
_close( $file_handler );
	...
	protected function replace_table_values( $input, $parse = false ) {
		...
		if ( $old_values ) {
			...
			return S2sm
_Database_Utility::replace_serialized_values( $old_values, $new_values, $input );
		}
	...
	protected function parse_serialized_values( $old_values, $new_values, $input ) {
		...
		if ( $this->number_of_replaces === 0 ) {
			$input = S2sm
_Database_Utility::replace_values( $old_values, $new_values, $input );
		}
	...
	protected function replace_serialized_values( $matches ) {
		...
		$input = S2sm
_Database_Utility::unescape_mysql( $matches[1] );

		// Replace serialized values
		$input = S2sm
_Database_Utility::replace_serialized_values( $this->get_old_replace_values(), $this->get_new_replace_values(), $input );
