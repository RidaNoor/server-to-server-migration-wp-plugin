...
_Http_Abstract $client = null ) {
		...
		// Get IP address
		$ip = get_option( S2SM_URL_IP );

		// Get adapter
		$adapter = get_option( S2SM_URL_ADAPTER );
...
_Http_Factory::create( $adapter );
		...
		if ( ( $user = get_option( S2SM_AUTH_USER ) ) && ( $password = get_option( S2SM_AUTH_PASSWORD ) ) ) {
...
_cache_flush();
				...
				// Is valid adapter?
				if ( get_option( S2SM_URL_IP ) && get_option( S2SM_URL_ADAPTER ) ) {
					return;
				}
